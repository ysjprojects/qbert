...
from .configuration_qbert import QBertConfig
...
class QuaternionTransformation(nn.Module):
    ...
    def __init__(self, input_dim, output_dim, activation=None, init=None):
        ...
        assert output_dim % 4 == 0, "Output dimension must be divisible by 4"
    ...
    def forward(self, x):
        ...
        if is_3d_input:
            ...
            batch_size, seq_len, output_dim = x.shape
...
class QBertSelfAttention(nn.Module):
    def __init__(self, config, position_embedding_type=None):
        ...
        if config.hidden_size % 4 != 0:
            raise ValueError(
                f"The hidden size ({config.hidden_size}) is not a multiple of 4"
            )

        self.num_attention_heads = config.num_attention_heads
        self.attention_head_size = int(config.hidden_size / config.num_attention_heads)
        if self.attention_head_size % 4 != 0:
            raise ValueError(
                f"The attention head size ({self.attention_head_size}) is not a multiple of 4"
            )
...
QBERT_SELF_ATTENTION_CLASSES = {
    "eager": QBertSelfAttention,
}
...
class QBertAttention(nn.Module):
    def __init__(self, config, position_embedding_type=None):
        super().__init__()
        self.self = QBERT_SELF_ATTENTION_CLASSES[config._attn_implementation](
...
class QBertPredictionHeadTransform(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.dense = nn.Linear(config.hidden_size, config.hidden_size)
        if isinstance(config.hidden_act, str):
...
class QBertLMPredictionHead(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.transform = QBertPredictionHeadTransform(config)
...
class QBertOnlyMLMHead(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.predictions = QBertLMPredictionHead(config)
...
class QBertOnlyNSPHead(nn.Module):
    def __init__(self, config):
        super().__init__()
...
class QBertPreTrainingHeads(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.predictions = QBertLMPredictionHead(config)
        self.seq_relationship = nn.Linear(config.hidden_size, 2)
...
class QBertPreTrainedModel(PreTrainedModel):
    """
...
BERT_START_DOCSTRING = r"""
    ...
    Parameters:
        config ([`QBertConfig`]): Model configuration class with all the parameters of the model.
...
class QBertModel(QBertPreTrainedModel):

    _no_split_modules = ["QBertEmbeddings", "QBertLayer"]
    ...
    def __init__(self, config, add_pooling_layer=True):
        ...
        self.pooler = QBertPooler(config) if add_pooling_layer else None
...
class QBertForPreTraining(QBertPreTrainedModel):
    ...
    def forward(
        ...
        output_hidden_states: Optional[bool] = None,
        return_dict: Optional[bool] = None,
    ) -> Union[Tuple[torch.Tensor], QBertForPreTrainingOutput]:
        r"""
        ...
        return QBertForPreTrainingOutput(
            loss=total_loss,
...
class QBertForMaskedLM(QBertPreTrainedModel):
    _tied_weights_keys = ["predictions.decoder.bias", "cls.predictions.decoder.weight"]
...
class QBertForNextSentencePrediction(QBertPreTrainedModel):
    def __init__(self, config):
        super().__init__(config)
...
class QBertForSequenceClassification(QBertPreTrainedModel):
    def __init__(self, config):
        super().__init__(config)
...
class QBertForMultipleChoice(QBertPreTrainedModel):
    def __init__(self, config):
        super().__init__(config)
...
class QBertForTokenClassification(QBertPreTrainedModel):
    def __init__(self, config):
        super().__init__(config)
        self.num_labels = config.num_labels
...
class QBertForQuestionAnswering(QBertPreTrainedModel):
    def __init__(self, config):
        super().__init__(config)
        self.num_labels = config.num_labels
...
__all__ = [
    "QBertForMaskedLM",
    "QBertForMultipleChoice",
    "QBertForNextSentencePrediction",
    "QBertForPreTraining",
    "QBertForQuestionAnswering",
    "QBertForSequenceClassification",
    "QBertForTokenClassification",
    "QBertLayer",
    "QBertModel",
